...
### Viewer
While the graphical output in the Client window was designed to give the players and the listeners visual feedback on musical and control events, the Viewer uses the Jitter graphical environment for the creation of a visual layer complementing a Quintet.net performance.

To demonstrate some basic features of the Viewer, the score of Five will be displayed. Launch Quintet.net Viewer 2020 and register the component with the Quintet.net Server. Then click the “Load Settings...” button and locate the Five.viewer.xml file in the Five project folder. Press shift-return to load the first preset. During a performance, the Conductor can be programmed to take care of this, but it's a good idea to try this first manually.
In the Viewer preferences, set the video resolution to resonable numbers corresponding to the aspect ratio of your projector or screen (a second screen is strongly recommended when working with the Viewer), move the Quintet.net window to the other screen and press "esc", which will put the Viewer in full-screen mode. 
